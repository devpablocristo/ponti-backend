...
func TestAxisProductIntegrationAuth_AllowsCapabilityExecutionsWithTenant(t *testing.T) {
	t.Setenv("X_API_KEY", "backend-key")
	t.Setenv("PONTI_AXIS_API_KEY", "axis-key")
	gin.SetMode(gin.TestMode)
	router := gin.New()
	...
	router.POST("/api/v1/capability-executions", handlers...)

	req := httptest.NewRequest(http.MethodPost, "/api/v1/capability-executions", nil)
	req.Header.Set("Authorization", "Bearer axis-key")
	req.Header.Set("X-Tenant-Id", uuid.NewString())
	res := httptest.NewRecorder()
	router.ServeHTTP(res, req)
