...
func NewGroupListResponse(pageInfo types.PageInfo, items []domain.WorkOrderDraftGroupListItem) *WorkOrderDraftGroupListResponse {
	resp := make([]WorkOrderDraftGroupListItem, len(items))
	for i, item := range items {
		resp[i] = WorkOrderDraftGroupListItem{
			ID:            item.ID,
			Number:        item.Number,
	...
	return &WorkOrderDraftGroupListResponse{
		PageInfo: pageInfo,
		Data:     resp,
	}
}
...
func GroupFromDomain(d *domain.WorkOrderDraftGroup) *WorkOrderDraftGroupResponse {
	items := make([]WorkOrderDraftResponseItem, len(d.Items))
	for i, item := range d.Items {
		items[i] = WorkOrderDraftResponseItem{
			SupplyID:   item.SupplyID,
	...
	lots := make([]WorkOrderDraftGroupLotResponse, len(d.Lots))
	for i, lot := range d.Lots {
		lots[i] = WorkOrderDraftGroupLotResponse{
			DraftID:       lot.DraftID,
			Number:        lot.Number,
			LotID:         lot.LotID,
			LotName:       lot.LotName,
	...
	return &WorkOrderDraftGroupResponse{
		ID:                   d.ID,
		Number:               d.Number,
		Date:                 d.Date.Format(dateLayout),
		CustomerID:           d.CustomerID,
...
type WorkOrderDraftGroupResponse struct {
	ID                   int64                                 `json:"id"`
	Number               string                                `json:"number"`
	Date                 string                                `json:"date"`
	CustomerID           int64                                 `json:"customer_id"`
	...
	PublishedWorkOrderID *int64                                `json:"published_work_order_id,omitempty"`
	ReviewNotes          string                                `json:"review_notes"`
	Lots                 []WorkOrderDraftGroupLotResponse      `json:"lots"`
	Items                []WorkOrderDraftResponseItem          `json:"items"`
	InvestorSplits       []WorkOrderDraftResponseInvestorSplit `json:"investor_splits,omitempty"`
	CreatedAt            string                                `json:"created_at"`
	UpdatedAt            string                                `json:"updated_at"`
...
type WorkOrderDraftGroupUpdateRequest struct {
	Number         string               `json:"number"`
	Date           string               `json:"date" binding:"required"`
	CustomerID     int64                `json:"customer_id" binding:"required"`
	ProjectID      int64                `json:"project_id" binding:"required"`
