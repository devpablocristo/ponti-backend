...
// TestLotRepositoryLegacyDeleteIsHardDelete verifica Fase H.1: el DeleteLot
// legacy ahora hace hard delete (antes hacía archive).
func TestLotRepositoryLegacyDeleteIsHardDelete(t *testing.T) {
	db := setupLotTenantDB(t)
	repo := NewRepository(lotTenantGormEngine{client: db})
	...
	// Crear y archivar (DeleteLot=HardDelete también requiere archived).
	lotID, err := repo.CreateLot(ctx, &domain.Lot{
		Name:     "LOTE LEGACY",
		FieldID:  10,
		Hectares: decimal.NewFromInt(1),
	})
	if err != nil {
		t.Fatalf("CreateLot: %v", err)
	}
	...
	// DeleteLot legacy → debería ser hard delete (no soft).
	if err := repo.DeleteLot(ctx, lotID); err != nil {
		t.Fatalf("DeleteLot legacy: %v", err)
	}

	// La fila ya no debe existir físicamente.
	var exists int64
	if err := db.Raw(`SELECT COUNT(*) FROM lots WHERE id = ?`, lotID).
		Scan(&exists).Error; err != nil {
		t.Fatalf("check legacy delete: %v", err)
	}
	if exists != 0 {
		t.Fatalf("DeleteLot legacy no borró físicamente (Fase H.1 falló)")
	}
}
