...
func TestHandleMovementInternalMovementOut_IncludesSupplyNameInError(t *testing.T) {
	...
	supplyID := int64(123)
	mockRepo.EXPECT().
		ProjectExists(gomock.Any(), destinationProjectID).
		Return(true, nil)
	mockRepo.EXPECT().
...
func TestHandleMovementInternalMovementOut_ReusesSupplyWhenAlreadyExistsInDestination(t *testing.T) {
	...
	assert.Equal(t, destinationProjectID, outMovementCreated.ProjectDestinationId)
	assert.Equal(t, int64(20), inMovementCreated.ProjectDestinationId)
}
...
func TestHandleMovementInternalMovementOut_CreatesSupplyInDestinationAndUsesIt(t *testing.T) {
	...
	gomock.InOrder(
		mockRepo.EXPECT().
			ProjectExists(gomock.Any(), destProjectID).
			Return(true, nil),
		mockRepo.EXPECT().
			GetSupply(gomock.Any(), originSupplyID).
	...
	assert.Equal(t, destProjectID, outMovementCreated.ProjectDestinationId)
	assert.Equal(t, destSupplyID, inMovementCreated.Supply.ID)
	assert.True(t, inMovementCreated.Quantity.Equal(decimal.NewFromInt(10)))
	assert.Equal(t, domain.INTERNAL_MOVEMENT_IN, inMovementCreated.MovementType)
	assert.Equal(t, originProjectID, inMovementCreated.ProjectDestinationId)
}

func TestCreateSupplyMovement_InternalMovementWithoutStock_ReturnsValidation(t *testing.T) {
	ctrl := gomock.NewController(t)
	t.Cleanup(ctrl.Finish)
	...
	mockRepo.EXPECT().
		ExistsSupplyMovementByProjectReferenceAndSupply(gomock.Any(), projectID, "INT-NIL", supplyID).
		Return(false, nil)
	mockRepo.EXPECT().
		GetSupply(gomock.Any(), supplyID).
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "No hay stock disponible del insumo")
}
...
func TestImportSupplyMovements_InternalMovementFailsWhenDestinationEqualsOrigin(t *testing.T) {
	ctrl := gomock.NewController(t)
	t.Cleanup(ctrl.Finish)
	...
	ids, failures, err := u.ImportSupplyMovements(context.Background(), []*domain.SupplyMovement{{
		ProjectId:            projectID,
		ProjectDestinationId: projectID,
		Quantity:             decimal.NewFromInt(2),
		MovementType:         domain.INTERNAL_MOVEMENT,
		MovementDate:         &date,
	...
	if assert.Len(t, failures, 1) {
		assert.Equal(t, 2, failures[0].RowIndex)
		assert.Equal(t, "validation_error", failures[0].Code)
		assert.Equal(t, "project_destination_id must be different from project_id", failures[0].Message)
		assert.Equal(t, "INT-001", failures[0].ReferenceNumber)
		assert.Equal(t, "Urea", failures[0].SupplyName)
	}
}

func TestImportSupplyMovements_InternalMovementFailsWhenDestinationProjectDoesNotExist(t *testing.T) {
	ctrl := gomock.NewController(t)
	t.Cleanup(ctrl.Finish)
	...
	if assert.Len(t, failures, 1) {
		assert.Equal(t, 2, failures[0].RowIndex)
		assert.Equal(t, "validation_error", failures[0].Code)
		assert.Equal(t, "El proyecto destino 99 no existe", failures[0].Message)
		assert.Equal(t, "INT-002", failures[0].ReferenceNumber)
		assert.Equal(t, "Urea", failures[0].SupplyName)
	}
}
...
func TestCreateSupplyMovement_ReturnMovement_CreatesNegativeEntry(t *testing.T) {
	ctrl := gomock.NewController(t)
	t.Cleanup(ctrl.Finish)
	...
	providerID := int64(5)
	stockID := int64(41)
	date := mustTime(t, "2026-03-05T00:00:00Z")

	mockRepo.EXPECT().
		ExistsSupplyMovementByProjectReferenceSupplyAndType(
			gomock.Any(),
			projectID,
			"DEV-001",
			supplyID,
	...
	var created *domain.SupplyMovement
	mockRepo.EXPECT().
		CreateSupplyMovement(gomock.Any(), gomock.Any()).
		DoAndReturn(func(_ context.Context, movement *domain.SupplyMovement) (int64, error) {
			created = movement
