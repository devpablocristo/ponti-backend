...
// vive en el wire y ya no hay cliente legacy paralelo.
//
// Notas críticas:
...
// Do enruta la llamada según el path. Los paths se mantienen estables para que
// el handler no cambie.
...
// devuelve al FE. Mantenemos campos de compatibilidad para no romper el FE.
func marshalCompanionChat(r *axis.ChatResponse) (int, []byte, error) {
	out := map[string]any{
		...
		// Campos de compatibilidad que el FE puede leer; los completamos con
...
// para que el handler maneje errores de upstream de forma uniforme.
