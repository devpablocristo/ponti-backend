...
//
// Históricamente este paquete envolvía el cliente HTTP de `ponti-ai` (FastAPI
// Python) con un fallback dummy cuando la URL no estaba configurada. Esa capa
// se removió post-cutover a `axis/companion`: la única ruta hoy es Companion,
// inyectado vía `ai.CompanionAdapter` que implementa `ClientPort`.
package usecases
