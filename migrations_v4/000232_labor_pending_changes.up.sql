...
-- Labor: agregar columna is_pending
ALTER TABLE public.labors
    ADD COLUMN IF NOT EXISTS is_pending BOOLEAN NOT NULL DEFAULT FALSE;

-- Labor: hacer category_id nullable y price con default 0
ALTER TABLE public.labors ALTER COLUMN category_id DROP NOT NULL;
ALTER TABLE public.labors ALTER COLUMN price SET DEFAULT 0;

-- Vista: workorder_list con fallback de contratista desde la labor
DROP VIEW IF EXISTS v4_report.workorder_list;

CREATE VIEW v4_report.workorder_list AS
SELECT
  w.id, w.number, w.project_id, w.field_id,
  p.name AS project_name, f.name AS field_name, l.name AS lot_name,
  w.date, c.name AS crop_name,
  lb.name AS labor_name, cat_lb.name AS labor_category_name,
  t.name AS type_name,
  COALESCE(NULLIF(w.contractor, ''), lb.contractor_name) AS contractor,
  w.effective_area AS surface_ha,
  s.name AS supply_name, wi.total_used AS consumption,
  cat.name AS category_name, wi.final_dose AS dose_per_ha,
  ...
  w.sequence_day, false::boolean AS is_digital, 'published'::varchar(30) AS status
...
SELECT
  ...
  COALESCE(NULLIF(w.contractor, ''), lb.contractor_name) AS contractor,
  w.effective_area AS surface_ha,
  lb.name::varchar(100) AS supply_name, 0::numeric(18,6) AS consumption,
  cat_lb.name::varchar(250) AS category_name, 0::numeric(18,6) AS dose_per_ha,
...
LEFT JOIN public.categories cat_lb ON cat_lb.id = lb.category_id AND cat_lb.deleted_at IS NULL
WHERE w.deleted_at IS NULL
  AND w.effective_area IS NOT NULL AND w.effective_area > 0 AND lb.price IS NOT NULL

UNION ALL

SELECT
  -wod.id AS id, wod.number, wod.project_id, wod.field_id,
  p.name AS project_name, f.name AS field_name, l.name AS lot_name,
  wod.date, c.name AS crop_name,
  lb.name AS labor_name, cat_lb.name AS labor_category_name,
  t.name AS type_name,
  COALESCE(NULLIF(wod.contractor, ''), lb.contractor_name) AS contractor,
  wod.effective_area AS surface_ha,
  s.name AS supply_name, wodi.total_used AS consumption,
  cat.name AS category_name, wodi.final_dose AS dose_per_ha,
  s.price AS unit_price,
  CASE
    ...
    ELSE 0
  END AS supply_cost_per_ha,
  v4_core.supply_cost(wodi.final_dose::numeric, s.price::numeric, wod.effective_area::numeric) AS supply_total_cost,
  0::integer AS sequence_day, true::boolean AS is_digital, wod.status::varchar(30) AS status
...
LEFT JOIN public.categories cat_lb ON cat_lb.id = lb.category_id AND cat_lb.deleted_at IS NULL
JOIN public.work_order_draft_items wodi ON wodi.draft_id = wod.id AND wodi.deleted_at IS NULL
...
LEFT JOIN public.categories cat_lb ON cat_lb.id = lb.category_id AND cat_lb.deleted_at IS NULL
WHERE wod.deleted_at IS NULL AND wod.is_digital = true AND wod.status = 'draft'
  AND wod.effective_area IS NOT NULL AND wod.effective_area > 0 AND lb.price IS NOT NULL;

COMMENT ON VIEW v4_report.workorder_list IS 'Listado detallado de workorders publicadas y drafts digitales abiertos usando funciones v4_core';
