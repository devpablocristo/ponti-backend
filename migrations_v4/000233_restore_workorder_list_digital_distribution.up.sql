...
SELECT
  ...
  CASE
    WHEN COALESCE(w.is_digital, false) = true
     AND wi.final_dose IS NOT NULL
     AND wi.final_dose > 0
    THEN ROUND((wi.final_dose * w.effective_area)::numeric, 2)
    ELSE wi.total_used
  END AS consumption,
  ...
  w.sequence_day,
  COALESCE(w.is_digital, false)::boolean AS is_digital,
...
-- Filas de insumos de drafts digitales abiertos.
SELECT
  -wod.id AS id,
  wod.number,
  wod.project_id,
  ...
  s.name AS supply_name,
  CASE
    WHEN wodi.final_dose IS NOT NULL
     AND wodi.final_dose > 0
    THEN ROUND((wodi.final_dose * wod.effective_area)::numeric, 2)
    ELSE wodi.total_used
  END AS consumption,
  cat.name AS category_name,
...
-- Fila de costo de labor de drafts digitales abiertos.
SELECT
  -wod.id AS id,
  wod.number,
  wod.project_id,
