-- Revierte 000231: restaura las vistas a su definición previa (000229), donde
-- yield_tn_per_ha se calcula sobre seeded_area_ha (no sobre hectáreas totales).
BEGIN;
...
base AS (
  SELECT
    ...
    v4_core.per_ha(r.tons, COALESCE(wt.seeded_area_ha, 0))::numeric AS yield_tn_per_ha,
...
SELECT
  ...
  v4_ssot.total_costs_for_project(p.id) AS operating_result_total_costs_usd,
...
GROUP BY p.customer_id, p.id, p.campaign_id, p.planned_cost, ph.total_hectares;
...
SELECT
  ...
  COALESCE(dc.direct_costs_total_usd, 0)::numeric
    + COALESCE(rt.rent_total_usd, 0)::numeric
    + COALESCE(ac.admin_total_usd, 0)::numeric AS operating_result_total_costs_usd,
  v4_ssot.renta_pct(
    COALESCE(it.income_net_total_usd, 0)::numeric
      - COALESCE(dc.direct_costs_total_usd, 0)::numeric
      - COALESCE(rt.rent_total_usd, 0)::numeric
