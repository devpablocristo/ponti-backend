...
-- Debe ser INNER JOIN: si la OT no tiene insumos, no debe aparecer una fila fantasma.
SELECT
  w.id,
  w.number,
  w.project_id,
...
-- Debe ser INNER JOIN: si el draft no tiene insumos, no debe aparecer una fila fantasma.
SELECT
  -wod.id AS id,
  wod.number,
  wod.project_id,
...
LEFT JOIN public.categories cat_lb ON cat_lb.id = lb.category_id AND cat_lb.deleted_at IS NULL
JOIN public.work_order_draft_items wodi ON wodi.draft_id = wod.id
