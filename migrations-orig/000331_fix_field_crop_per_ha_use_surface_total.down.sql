-- =============================================================================
-- MIGRACIÓN 000331: Rollback per-ha usa superficie total
-- =============================================================================
--
-- Propósito: Restaurar divisores por área sembrada (sowed_area_ha)
-- Nota: Comentarios en español, código en inglés
--

BEGIN;

-- 1) lot_base_costs: costo por ha usa área sembrada
...
SELECT
  ...
  v4_core.per_ha(direct_cost_usd, sowed_area_ha) AS direct_cost_per_ha_usd,
  rent_per_ha_usd,
...
-- 2) field_crop_labores: per_ha usa área sembrada
...
labor_costs AS (
  ...
  GROUP BY lb.project_id, lb.field_id, lb.crop_id, lb.lot_id, lb.sowed_area_ha
)
SELECT
  project_id,
  field_id,
...
-- 3) field_crop_insumos: per_ha usa área sembrada
...
-- 4) field_crop_economicos: per_ha usa área sembrada y arriendo total visible
...
-- 5) field_crop_rentabilidad: per_ha usa área sembrada
...
-- 6) field_crop_metrics: versión original (divide por superficie sembrada)
...
-- 7) summary_results: versión original
