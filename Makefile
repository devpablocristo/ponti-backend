...
define compose_cmd
PLATFORM_REPO_TOKEN="$(PLATFORM_REPO_TOKEN)" docker compose -f $(DOCKER_COMPOSE_YML)
endef
...
actors-backfill-sync:
	...
	@if [ ! -f scripts/db/actors_backfill_sync.sql ]; then \
		echo "WARN: scripts/db/actors_backfill_sync.sql no existe en esta rama; no hay nada para ejecutar."; \
	else \
...
dev:
	...
	@if [ -z "$(PLATFORM_REPO_TOKEN)" ]; then echo "WARN: PLATFORM_REPO_TOKEN vacio. Si el cache de Go esta frio, ponti-api no podra bajar github.com/devpablocristo/platform/*."; fi
