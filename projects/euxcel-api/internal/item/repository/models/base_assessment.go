...
// Item representa la evaluación en la capa GORM.
type Item struct {
...
type SkillConfig struct {
	ID           string    `gorm:"primaryKey"`
	AssessmentID string    `gorm:"index;not null"`             // Foreign Key a Item
...
func (dto Item) ToDomain() *domain.Item {
	return &domain.Item{
		ID:          dto.ID,
		HRID:        dto.HRID,
...
func FromDomainAssessment(item *domain.Item) *Item {
	var endDatePtr *time.Time
	if !item.EndDate.IsZero() {
		endDatePtr = &item.EndDate
	}
	...
	return &Item{
		...
		MaxDuration: int64(item.MaxDuration.Minutes()),
		Skills:      SkillConfigFromDomainAssessment(item.Skills),
		Problem:     ProblemFromDomainAssessment(&item.Problem),
		UnitTests:   UnitTestFromDomainAssessment(item.UnitTests),
	}
}
