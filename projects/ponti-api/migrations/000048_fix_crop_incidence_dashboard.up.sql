-- =========================================================
-- MIGRACIÓN 000047: Corregir crop_incidence en get_dashboard_payload
-- =========================================================
-- FECHA: 2025-01-29
-- DESCRIPCIÓN: Corregir la función get_dashboard_payload para que incluya
--              la lógica real de crop_incidence en lugar de valores hardcodeados
-- =========================================================

-- Crear la función get_dashboard_payload corregida que incluye crop_incidence real
...
-- =========================================================
-- 2) CROP INCIDENCE - Lógica real para cultivos
-- =========================================================
crop_data AS (
  SELECT 
    c.name,
    COALESCE(SUM(l.hectares), 0)::numeric(14,2) AS hectares,
    -- Calcular % de rotación basado en área total
    CASE WHEN (SELECT COALESCE(SUM(l2.hectares), 0) FROM lots l2 
               JOIN fields f2 ON l2.field_id = f2.id 
               JOIN projects p2 ON f2.project_id = p2.id
               WHERE (f.customer_id IS NULL OR p2.customer_id = f.customer_id)
                 AND (f.project_id IS NULL OR p2.id = f.project_id)
                 AND (f.campaign_id IS NULL OR p2.campaign_id = f.campaign_id)
                 AND (f.field_id IS NULL OR f2.id = f.field_id)) > 0
         THEN ROUND((SUM(l.hectares) / (SELECT COALESCE(SUM(l2.hectares), 0) FROM lots l2 
                                        JOIN fields f2 ON l2.field_id = f2.id 
                                        JOIN projects p2 ON f2.project_id = p2.id
                                        WHERE (f.customer_id IS NULL OR p2.customer_id = f.customer_id)
                                          AND (f.project_id IS NULL OR p2.id = f.project_id)
                                          AND (f.campaign_id IS NULL OR p2.campaign_id = f.campaign_id)
                                          AND (f.field_id IS NULL OR f2.id = f.field_id)) * 100)::numeric, 2)
         ELSE 0 END AS rotation_pct,
    -- Calcular costo por hectárea (simplificado - solo labores por ahora)
    CASE WHEN COALESCE(SUM(l.hectares), 0) > 0 
         THEN ROUND((COALESCE((
           SELECT COALESCE(SUM(lab.price), 0) 
           FROM labors lab 
           JOIN workorders wo ON lab.id = wo.labor_id
           JOIN lots l3 ON wo.lot_id = l3.id 
           WHERE l3.current_crop_id = c.id
             AND (f.project_id IS NULL OR wo.project_id = f.project_id)
             AND (f.field_id IS NULL OR l3.field_id = f.field_id)
         ), 0) / SUM(l.hectares))::numeric, 2)
         ELSE 0 END AS cost_usd_per_ha,
    -- Calcular % de incidencia del cultivo
    CASE WHEN (SELECT COALESCE(SUM(l2.hectares), 0) FROM lots l2 
               JOIN fields f2 ON l2.field_id = f2.id 
               JOIN projects p2 ON f2.project_id = p2.id
               WHERE (f.customer_id IS NULL OR p2.customer_id = f.customer_id)
  ...
  FROM crops c
  LEFT JOIN lots l ON c.id = l.current_crop_id
  LEFT JOIN fields f2 ON l.field_id = f2.id
  LEFT JOIN projects p2 ON f2.project_id = p2.id, flt f
  WHERE (f.customer_id IS NULL OR p2.customer_id = f.customer_id)
    AND (f.project_id IS NULL OR p2.id = f.project_id)
    AND (f.campaign_id IS NULL OR p2.campaign_id = f.campaign_id)
    AND (f.field_id IS NULL OR f2.id = f.field_id)
  GROUP BY c.id, c.name, f.customer_id, f.project_id, f.campaign_id, f.field_id
  HAVING COALESCE(SUM(l.hectares), 0) > 0  -- Solo cultivos con hectáreas > 0
),
crop_incidence_data AS (
  SELECT 
    jsonb_agg(
      jsonb_build_object(
        'name', cd.name,
        'hectares', cd.hectares,
        'rotation_pct', cd.rotation_pct,
        'cost_usd_per_ha', cd.cost_usd_per_ha,
        'incidence_pct', cd.incidence_pct
      )
    ) AS crops,
    COALESCE(SUM(cd.hectares), 0)::numeric(14,2) AS total_hectares,
    CASE WHEN (SELECT COALESCE(SUM(l2.hectares), 0) FROM lots l2 
               JOIN fields f2 ON l2.field_id = f2.id 
               JOIN projects p2 ON f2.project_id = p2.id, flt f
               WHERE (f.customer_id IS NULL OR p2.customer_id = f.customer_id)
                 AND (f.project_id IS NULL OR p2.id = f.project_id)
                 AND (f.campaign_id IS NULL OR p2.campaign_id = f.campaign_id)
                 AND (f.field_id IS NULL OR f2.id = f.field_id)) > 0
         THEN 100
         ELSE 0 END AS total_rotation_pct,
    CASE WHEN (SELECT COALESCE(SUM(l2.hectares), 0) FROM lots l2 
               JOIN fields f2 ON l2.field_id = f2.id 
               JOIN projects p2 ON f2.project_id = p2.id, flt f
               WHERE (f.customer_id IS NULL OR p2.customer_id = f.customer_id)
                 ...
                 AND (f.campaign_id IS NULL OR p2.campaign_id = f.campaign_id)
                 AND (f.field_id IS NULL OR f2.id = f.field_id)) > 0
         THEN ROUND((COALESCE(SUM(cd.hectares * cd.cost_usd_per_ha), 0) / 
                     (SELECT COALESCE(SUM(l2.hectares), 0) FROM lots l2 
                      JOIN fields f2 ON l2.field_id = f2.id 
                      JOIN projects p2 ON f2.project_id = p2.id, flt f
                      WHERE (f.customer_id IS NULL OR p2.customer_id = f.customer_id)
                        AND (f.project_id IS NULL OR p2.id = f.project_id)
                        AND (f.campaign_id IS NULL OR p2.campaign_id = f.campaign_id)
                        AND (f.field_id IS NULL OR f2.id = f.field_id)))::numeric, 2)
         ELSE 0 END AS total_cost_usd_per_hectare
  FROM crop_data cd
)

-- =========================================================
-- 3) COMPOSICIÓN DEL JSON SIMPLIFICADO - SIEMPRE DEVUELVE UN JSON VÁLIDO
-- =========================================================
SELECT COALESCE(
  jsonb_build_object(
    ...
    'crop_incidence', jsonb_build_object(
      'crops', COALESCE(cid.crops, '[]'::jsonb),
      'total', jsonb_build_object(
        'hectares', COALESCE(cid.total_hectares, 0),
        'rotation_pct', COALESCE(cid.total_rotation_pct, 0),
        'cost_usd_per_hectare', COALESCE(cid.total_cost_usd_per_hectare, 0)
      )
    ),
...
FROM sowing_kpi sk, harvest_kpi hk, costs_kpi ck, contribs c, oper_card oc, crop_incidence_data cid;
$$;
