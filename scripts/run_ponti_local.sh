...
wait_http_ok() {
  ...
  until http_ok_from_host "$url"; do
...
if [[ -f "$WEB_DIR/docker-compose.yml" ]]; then
  ...
  if docker compose --progress quiet -f "$WEB_DIR/docker-compose.yml" ps -q | grep -q .; then
    docker compose --progress quiet -f "$WEB_DIR/docker-compose.yml" kill >/dev/null 2>&1 || true
  fi
  docker compose --progress quiet -f "$WEB_DIR/docker-compose.yml" down --remove-orphans --timeout 1 || true
fi
...
if ! wait_http_ok "http://localhost:8080/api/v1/ping" 90; then
